					...
					<%@ taglib prefix="carbon" uri="http://wso2.org/projects/carbon/taglibs/carbontags.jar" %>
						...
						<body>
							...
							<% if("Success".equals(request.getParameter("addTenant"))){ %>
								...
								<% } String tenantdomain=(String)request.getAttribute(MultitenantConstants.TENANT_DOMAIN); String tip=""
									; if(tenantdomain==null){ tenantdomain="" ;
									tip="Enter the username in the format: user  name@yourdomain, Example: foo@abc.com" ; } else{
									tenantdomain="@" + tenantdomain; } String userForumURL=(String)
									config.getServletContext().getAttribute(CarbonConstants.PRODUCT_XML_WSO2CARBON +
									CarbonConstants.PRODUCT_XML_USERFORUM); String userGuideURL=(String)
									config.getServletContext().getAttribute(CarbonConstants.PRODUCT_XML_WSO2CARBON +
									CarbonConstants.PRODUCT_XML_USERGUIDE); String mailinglistURL=(String)
									config.getServletContext().getAttribute(CarbonConstants.PRODUCT_XML_WSO2CARBON +
									CarbonConstants.PRODUCT_XML_MAILINGLIST); String issuetrackerURL=(String)
									config.getServletContext().getAttribute(CarbonConstants.PRODUCT_XML_WSO2CARBON +
									CarbonConstants.PRODUCT_XML_ISSUETRACKER); if (userForumURL==null) { userForumURL="#" ; } if
									(userGuideURL==null) { userGuideURL="#" ; } if (mailinglistURL==null) { mailinglistURL="#" ; } if
									(issuetrackerURL==null) { issuetrackerURL="#" ; } %>
									<fmt:bundle basename="org.wso2.carbon.i18n.Resources">
										<% String loginStatus=request.getParameter("loginStatus"); if (loginStatus !=null && "false"
											.equalsIgnoreCase(loginStatus)) { %>
											<script type="text/javascript">
												jQuery(document).ready(function () {
											...
											<% } if (loginStatus !=null && "failed" .equalsIgnoreCase(loginStatus)) { %>
												...
												<% } String backendURL=CarbonUIUtil.getServerURL(config.getServletContext(), session); String
													registerPath=null; int serviceIndex=backendURL.indexOf("/services/"); int
													startingIndex=backendURL.contains("https")? "https://" .length(): "http://" .length(); String
													serverName=backendURL.substring(startingIndex, serviceIndex); int
													dotIndex=serverName.indexOf("."); if (dotIndex !=-1) { String
													managerServerName=serverName.substring(dotIndex + 1); registerPath="https://" +
													managerServerName + "/carbon/tenant-register/select_domain.jsp" ; } %>
													<%--New page design begings--%>
														<link rel="stylesheet" type="text/css" href="../tenant-login/css/loginpage.css" />

														<div id="login-content">
															<div class="main-text">
																The 0CodeAPI Manager, supporting management of all aspects of an API and its life cycle,
																				...
																				` <tr>
																					<td class="user_pass_td">
																						...
																						<form action='../admin/login_action.jsp' method="POST" target="_self">
																							...
																							<input type="hidden" id="txtbackendURL" name="backendURL" class="user"
																								tabindex="1" value="<%=backendURL%>" />
																							<table class="user_pass_table">
																								...
																								<tr>
																									<td></td>
																									<td><input type="submit" value="<fmt:message key=" sign.in" />"
																										tabindex="3"/></td>
																								</tr>
																								<% if (registerPath !=null) { %>
																									<tr>
																										<td colspan="2">
																											If your organization doesn't have an account, please <a
																												href="<%=registerPath%>" alt="Go To Register Page">register</a>.
																										</td>
																									</tr>
																									<% } %>
																										<tr>
																											<td colspan="2"><a class="help" target="_blank"
																													href="../tenant-login/docs/userguide.html" tabindex="4">
																													<fmt:message key="sign.in.help" />
																												</a>
																												&nbsp;|&nbsp;<a tabindex="5"
																													href="../admin-mgt/forgot_password.jsp" target="_blank"
																													class="password">
																						...
																						<div id="loginBoxGoogle">
																							...
																							<form action="../relyingparty/openid.jsp" name="googleAppLogin"
																								...
																								<table class="styledLeft noBorders">
																									<tbody>
																										<tr>
																											<td><input type="text" name="gAppDomainOpenId" tabindex="3" />
																											</td>
																											<td><input type="button" class="button" value="GO"
															...
															<div id="footer">
																<div class="powered">
																	<span>Powered by</span><a target="_blank" href="http://wso2.org/projects/wsas"><img
